...
import { ExternalLink, Loader2, PlayCircle, FolderOpen, Bookmark, CalendarCheck, CheckCircle2, Circle } from 'lucide-react'
...
export function ProblemDetailsDialog({ questionId, open, onOpenChange, onUpdate }: ProblemDetailsDialogProps) {
      ...
      <DialogContent aria-describedby={undefined} className="sm:max-w-[100vw] max-w-[100vw] w-screen h-[100dvh] m-0 rounded-none p-0 flex flex-col bg-background/95 backdrop-blur-xl border-none">
        ...
        {isLoading || !details ? (
          ...
          <div className="flex flex-col h-full overflow-hidden">
            {/* Header */}
            <div className="p-6 md:p-8 border-b border-border bg-card/30 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-4">
                {details.question.leetcode_number && (
                  <span className="font-mono text-muted-foreground bg-muted px-2 py-1 rounded-md text-sm">
                    {details.question.leetcode_number}
                  </span>
                )}
                <h1 className="text-2xl md:text-3xl font-bold">{details.question.title}</h1>
                <Badge variant="outline" className={
                  details.question.difficulty === 'Easy' ? 'border-emerald-500/30 text-emerald-500' :
                  details.question.difficulty === 'Medium' ? 'border-amber-500/30 text-amber-500' :
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="p-2 hover:bg-muted rounded-full transition-colors text-blue-400 hover:text-blue-300"
                >
